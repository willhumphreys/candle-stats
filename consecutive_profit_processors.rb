...
class Consecutive_Profit_Processors
  ...
  def initialize
    ...
    @first_to_third_tick_wins = 0
    @four_consecutive_wins_count = 0

    winner_next_one_winner_l = lambda do |first, second, third, fourth|
      if first.ticks >0
        @first_tick_wins += 1
        if second.ticks > 0
    ...
    two_winners_next_one_winner_l = lambda do |first, second, third, fourth|
    ...
    three_win_next_one_wins_l = lambda do |first, second, third, fourth|
      if first.ticks >0 && second.ticks > 0 && third.ticks > 0
        @first_to_third_tick_wins += 1
        if fourth.ticks > 0
          @four_consecutive_wins_count += 1
        end
      end
      @four_consecutive_wins_count.to_f / @first_to_third_tick_wins
    end
    ...
    @processors = {
        :one_trade_wins_odds_next_trade_wins => Processor.new('one_trade_wins_odds_next_trade_wins', winner_next_one_winner_l),
        :two_trades_win_odds_next_trade_wins => Processor.new('two_trades_win_odds_next_trade_wins', two_winners_next_one_winner_l),
        :three_trades_win_odds_next_trade_wins => Processor.new('three_trades_win_odds_next_trade_wins', three_win_next_one_wins_l),
